...
namespace Fps
{
    ...
    public class MapBuilderV2Inspector : Editor
    {
        ...
        {
            var mapBuilder = (MapBuilderV2) target;
            var defaultTileCollectionProp = serializedObject.FindProperty(nameof(MapBuilderV2.DefaultTileCollection));


            EditorGUILayout.PropertyField(serializedObject.FindProperty(nameof(MapBuilderV2.MapTileLookupTexture)));

            EditorGUILayout.PropertyField(defaultTileCollectionProp);

            if (defaultTileCollectionProp.objectReferenceValue != null)
            {
                EditorGUILayout.PropertyField(serializedObject.FindProperty(nameof(MapBuilderV2.ExtraTileCollections)), true);
            }
